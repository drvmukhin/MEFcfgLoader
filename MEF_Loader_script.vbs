...
'-----------------------------------------------------------------
	...
	Select Case Err.Number
		...
		Case 70
		    Do 
				...
				Call RunCmd("127.0.0.1", "", vCmdOut, strCMD, objShell, nDebug)
			    strPID = ""
				...
				Else 
					...
					Call IE_MSG(vIE_Scale, "Error",vvMsg,2)
					Exit Sub
				End If
			Loop
		Case Else 
	...
	On Error Resume Next
		...
		if Err.Number <> 0 Then 
			...
			Call IE_MSG(vIE_Scale, "Error",vvMsg,8)
	...
	On Error Resume Next
		...
		if Err.Number <> 0 Then 
			...
			Call IE_MSG(vIE_Scale, "Error",vvMsg,3)
            SecureCRT_Installed = False
...
'##############################################################################
 Function IE_MSG (vIE_Scale, strTitle, ByRef vLine, ByVal nLine)
    Set g_objIE = Nothing
    Set objShell = Nothing
    Dim intX
    Dim intY
...
Function IE_PromptForInput(ByRef vIE_Scale, ByRef vSessionTmp, ByRef vSvc, ByRef vFlavors, byRef vSettings, ByRef vNodes, byRef vTemplates, byref vXLSheetPrefix, nDebug)
	...
	g_objIE.Document.body.Style.overflow = "hidden"
	g_objIE.Document.body.Style.border = "None " & HttpBdColor1
	g_objIE.Document.body.Style.background = "url('" & BgFigure & "')"
	g_objIE.Document.body.Style.color = HttpTextColor1
    g_objIE.height = WindowH
    g_objIE.width = WindowW  
	...
	nLine = 0
		...
		"<tr>" &_
			...
			"</td>" &_
				...
				"<button style='border-style: None; background: url(""" & SettingsFigure & """); width:" &_
				"36;height:36;" &_
...
'   g_objIE.Document.All("ConfigLocation")(0).Click	
	...
	For Each strLine in vCmdOut
	   If InStr(strLine,"iexplore.exe") then strPID = Split(strLine,""",""")(1)
	     ' Call TrDebug("READ TASK PID:" , strLine, objDebug, MAX_LEN, 1, 1)
    Next
    If strPID = "" Then Call GetAppPID(strPID, "iexplore.exe")
	objShell.AppActivate strPID			
	'----------------------------------------------------
   ...
   Do
        ...
        Select Case szNothing
		    Case "CONFIG_SOURCE"
						if g_objIE.Document.All("ConfigLocation").Checked then
							...
							If Not objFSO.FileExists(SourceFolder & "\" & vSvc(nService,1) & "\" & vFlavors(nService, nFlavor,0) & "-" & nTask & "-" & Platform & "-l.conf") Then
								...
								Call IE_MSG(vIE_Scale, "Can't find configuration",vvMsg, 3)
							    SourceFolder = strDirectoryConfig
							    Arg4 = "original"
			            ...
			            Do
							...
							If Not objFSO.FileExists(SourceFolder & "\" & vSvc(nNewService,1) & "\" & vFlavors(nNewService, nNewFlavor,0) & "-" & nNewTask & "-" & Platform & "-l.conf") Then
								...
								Else 
									...
									Call IE_MSG(vIE_Scale, "Can't find file",vvMsg, 3)
									g_objIE.document.getElementById("Input_Param_0").selectedIndex = nService
									Exit Do
								End If 
							End If
			...
			Case "APPLY_FWF"
						...
						Do
							...
							If g_objIE.document.getElementById("bw_profile").Disabled Then 
								...
								Call IE_MSG(vIE_Scale, "Applying BW Profile?",vvMsg, 3)
					    		Exit Do
							End If 
							...
							If nService <> CurrentSvc or nFlavor <> CurrentFlv or nTask <> CurrentTsk Then 
								...
								Call IE_MSG (vIE_Scale, "Applying BW Profile?", vvMsg, 2)
								Exit Do
							End If
							'---------------------------------------------------------
							' RUN TELNET SCRIPT
			...
			Case "DOWNLOAD"
			            ...
			            Do
    						...
    						If CurrentSvc = "Null" Then
								...
								If IE_CONT(vIE_Scale, "DownLoad configurations?", vvMsg, 4, nDebug) Then 
									CurrentSvc = nService
									CurrentFlv = nFlavor
									CurrentTsk = nTask
								End If						
							...
							If CurrentSvc <> "Null" Then
						        If Not YES_NO Then
									...
									If Not IE_CONT(vIE_Scale, "Downloading Final Configuration?", vvMsg, 3, nDebug) Then Exit Do
								Else 
			...
			Case "LOAD"
						...
						Do
							...
							If Not CFG_Downloaded and CurrentSvc <> "Null" Then 
								...
								If IE_CONT(vIE_Scale, "Continue?", vvMsg,3, nDebug) Then 
							...
							If IE_CONT(vIE_Scale, "Load new configurations?", vvMsg, 3, nDebug) Then 
								strConfigFileL = vFlavors(nService, nFlavor,0) & "-" & nTask & "-" & Platform & "-l.conf"
								strConfigFileR = vFlavors(nService, nFlavor,0) & "-" & nTask & "-" & Platform & "-r.conf"
								'---------------------------------------------------------
			...
			Case "POPULATE_ORIG"
				...
				If IE_CONT(vIE_Scale, "DownLoad configurations?", vvMsg, 3, nDebug) Then 
...
'--------------------------------------------------------------------
Function RunCmd(strHost, strPsExeFolder, ByRef vCmdOut, strCMD, ByRef objShell, nDebug)	
	Dim nResult, f_objFSO
